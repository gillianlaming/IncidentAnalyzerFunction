...
namespace IncidentAnalyzerFunction
{
    class AutoTriager
    {
        ...
        private async Task TestForHostingDbCpuIssue()
        {
            try
            {
                const double CpuThreshold = 0.85;
                const int MinutesOfCpuHighToDeclareCpuOutage = 5;
                ...
                {
                    tc.Result = TestCase.TestResult.ProblemDetected;
                    tc.ResultMessage.Add($"<br> - We detected that hosting db CPU above {CpuThreshold * 100}% for a period of {MinutesOfCpuHighToDeclareCpuOutage} minutes or longer.");
                    ...
                    tc.ActionSuggestions.Add($"&emsp; - We detected that hosting db CPU above {CpuThreshold * 100}% for a period of {MinutesOfCpuHighToDeclareCpuOutage} minutes or longer. Automorphism is supposed to handle database scaling. If it doesn't, follow this TSG:");
                }
        ...
        private async Task TestForAzureStorageIssue()
        {
            try
            {
                ...
                TestCase tc = new TestCase("TestForXStoreIssue");
